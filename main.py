...
from scripts.components import (
    kpi_tile, render_pivot, render_spend_sales_line, render_charts,
    render_targeting_breakdown, render_search_terms_tab,
    AD_TYPE_SUMMARY_FORMAT, WASTED_FORMAT, _pin_grand_total,
)
...
# ── Process data on button click, store in session_state ──
if run_audit:
...
_d = st.session_state["audit_data"]
sp_search = _d["sp_search"]
sp_place  = _d["sp_place"]
sb_search = _d["sb_search"]
sb_place  = _d["sb_place"]
sd_data   = _d["sd_data"]
sp_kpi    = _d["sp_kpi"]
sb_kpi    = _d["sb_kpi"]
...
# --- Performance Metrics with Ad Type Selector ---
st.markdown("### Performance Metrics")

# Build dataset map for KPI selector
kpi_datasets = {"Total (All Ad Types)": combined}
if not sp_kpi.empty:
    kpi_datasets["Sponsored Products"] = sp_kpi
if not sb_kpi.empty:
    kpi_datasets["Sponsored Brands"] = sb_kpi
if not sd_data.empty:
    kpi_datasets["Sponsored Display"] = sd_data

kpi_selection = st.selectbox(
    "View metrics for",
    list(kpi_datasets.keys()),
    key="kpi_ad_type",
)

kpi_data = kpi_datasets[kpi_selection]
summary = kpi_summary(kpi_data)
units_val = int(kpi_data["Units"].sum()) if "Units" in kpi_data.columns else 0
...
with tab_sp:
    ...
    # Sub-tabs for SP: Match Type / Placement / Search Terms
    sp_sub_tabs = ["Match Type Analysis", "Placements", "Search Terms"]
    sp_sub = st.tabs(sp_sub_tabs)
    ...
    with sp_sub[0]:
        if not sp_search.empty:
            ...
            # Auto campaign targeting breakdown
            st.markdown("---")
            render_targeting_breakdown(sp_search, "SP")
        else:
    ...
    with sp_sub[2]:
        render_search_terms_tab(sp_search)
...
with tab_sb:
    ...
    if not sb_search.empty:
        ...
        # Auto campaign targeting breakdown
        st.markdown("---")
        render_targeting_breakdown(sb_search, "SB")
    else:
...
with tab_trends:
    ...
    if rows:
        ...
        # Pin Grand Total and highlight it
        ad_df = _pin_grand_total(ad_df, "Ad Type")

        def _highlight_grand_total(row):
            if str(row.get("Ad Type", "")).strip() == "Grand Total":
                return ["font-weight: 700; background-color: #f1f5f9;"] * len(row)
            return [""] * len(row)

        st.dataframe(
            ad_df.style.format(
                {k: v for k, v in AD_TYPE_SUMMARY_FORMAT.items() if k in ad_df.columns}
    ...
    if wrows:
        ...
        # Pin Grand Total and highlight it
        w_df = _pin_grand_total(w_df, "Ad Type")

        def _highlight_gt_wasted(row):
            if str(row.get("Ad Type", "")).strip() == "Grand Total":
                return ["font-weight: 700; background-color: #f1f5f9;"] * len(row)
            return [""] * len(row)
        ...
        st.dataframe(
            w_df.style.format(
                {k: v for k, v in WASTED_FORMAT.items() if k in w_df.columns}
            ).apply(_highlight_gt_wasted, axis=1),
            use_container_width=True, hide_index=True,
        )
